...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	const wchar_t* src1[] = { L"One ", L"Two ", L"Three ", L"Four" };
	...
	const wchar_t* des1_temp = (*des1 != '\0') ? des1 : L"<empty>";
	wprintf(L"  des_len: %d (%d) des: %s \n", des1_len, wcslen(des1_temp), des1_temp);

	// Destination buffer too small
	const wchar_t* src2[] = { L"Red ", L"Green ", L"Blue ", L"Yellow " };
	...
	const wchar_t* des2_temp = (*des2 != '\0') ? des2 : L"<empty>";
	wprintf(L"  des_len: %d (%d) des: %s \n", des2_len, wcslen(des2_temp), des2_temp);

	// Empty string test
	const wchar_t* src3[] = { L"Airplane ", L"Car ", L"", L"Truck ", L"Boat " };
	...
	const wchar_t* des3_temp = (*des3 != '\0') ? des3 : L"<empty>";
